...
public class YamlConfigLoader implements
  ...
  private void loadProfileConfigFiles(PathMatchingResourcePatternResolver resolver,
      MutablePropertySources propertySources, ConfigurableEnvironment environment) throws IOException {
  ...
  private void loadProfileSpecificFiles(PathMatchingResourcePatternResolver resolver,
    ...
    Resource[] profileYamlResources = resolver.getResources(String.format(PROFILE_YAML_PATTERN, profile));
    Resource[] profileYmlResources = resolver.getResources(String.format(PROFILE_YML_PATTERN, profile));

    // Combine both arrays
    Resource[] allProfileResources = new Resource[profileYamlResources.length + profileYmlResources.length];
    System.arraycopy(profileYamlResources, 0, allProfileResources, 0, profileYamlResources.length);
    System.arraycopy(profileYmlResources, 0, allProfileResources, profileYamlResources.length, profileYmlResources.length);
