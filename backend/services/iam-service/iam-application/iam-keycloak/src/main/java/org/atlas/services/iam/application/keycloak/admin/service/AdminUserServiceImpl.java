...
import org.atlas.services.iam.port.out.keycloak.repository.KeycloakUserRepository;
import org.atlas.services.iam.port.out.repository.criteria.FindKeycloakUserCriteria;
...
public class AdminUserServiceImpl implements AdminUserService {
  ...
  private final ObjectProvider<KeycloakUserRepository> keycloakUserRepositoryProvider;
  ...
  public PagingResult<AdminUserOutput> retrieveUserList(AdminRetrieveUserListInput input) {
    ...
    PagingRequest pagingRequest = safePagingRequest(input.getPagingRequest());

    KeycloakUserRepository keycloakUserRepository = keycloakUserRepositoryProvider.getIfAvailable();
    if (keycloakUserRepository != null) {
      FindKeycloakUserCriteria criteria = FindKeycloakUserCriteria.builder()
          .realmName(keycloakProps.getRealm())
          ...
          .role(input.getRole())
          .build();
      PagingResult<UserEntity> userPage = keycloakUserRepository.findByCriteria(criteria, pagingRequest);
      return MapperUtil.mapPage(userPage, this::toAdminUserOutput);
    }

    if (StringUtil.isNotBlank(input.getUserId())) {
      AdminUserOutput user = retrieveUser(input.getUserId());
      return PagingResult.of(List.of(user), 1L, pagingRequest);
    }

    UsersResource users = realm().users();
    int first = pagingRequest.getOffset();
    int max = pagingRequest.getLimit();
    ...
    List<UserRepresentation> kcUsers = users.search(search, first, max);
    ...
    return PagingResult.of(outputs, totalRecords, pagingRequest);
  }

  private AdminUserOutput toAdminUserOutput(UserEntity user) {
  ...
  private boolean matches(AdminUserOutput user, AdminRetrieveUserListInput input) {
    if (input == null) {
      return true;
    }
    if (StringUtil.isNotBlank(input.getUsername())
        && !containsIgnoreCase(user.getUsername(), input.getUsername())) {
      return false;
    }
    if (StringUtil.isNotBlank(input.getFirstName())
        && !containsIgnoreCase(user.getFirstName(), input.getFirstName())) {
      return false;
    }
    if (StringUtil.isNotBlank(input.getLastName())
        && !containsIgnoreCase(user.getLastName(), input.getLastName())) {
      return false;
    }
    if (StringUtil.isNotBlank(input.getEmail())
        && !containsIgnoreCase(user.getEmail(), input.getEmail())) {
      return false;
    }
    if (StringUtil.isNotBlank(input.getPhoneNumber())
        && !containsIgnoreCase(user.getPhoneNumber(), input.getPhoneNumber())) {
      return false;
    }
    return true;
  }
  ...
  private PagingRequest safePagingRequest(PagingRequest pagingRequest) {
    if (pagingRequest == null) {
      return PagingRequest.of(0, CommonConstant.DEFAULT_PAGE_SIZE);
    }
    ...
    return PagingRequest.of(page, size, pagingRequest.getSortBy(), pagingRequest.getSortOrder());
  }
