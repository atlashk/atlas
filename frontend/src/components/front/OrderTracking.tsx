...
const OrderTracking: React.FC = () => {
   ...
   */
  const updateOrderStatus = useCallback((type: keyof OrderStatuses, status: OrderStatus, reason: string | null = null): void => {
   ...
   */
  const startShortPolling = useCallback((orderId: string): void => {
   ...
   * @param orderId - The order ID to track
   */
  const connectSSE = useCallback((orderId: string): void => {
    // Close existing connection if any
    if (eventSourceRef.current) eventSourceRef.current.close();

    // Create new SSE connection to order-specific endpoint
    eventSourceRef.current = new EventSource(`${API_BASE_URL}/notification/sse/orders/${orderId}/status`);
    ...
    eventSourceRef.current.addEventListener('ORDER_STATUS_CHANGED', (event: MessageEvent) => {
      try {
   ...
   * @param orderId - The order ID to track
   */
  const connectWebSocket = useCallback((orderId: string): void => {
    ...
    stompClientRef.current = new Client({
      ...
      onConnect: () => {
        ...
        stompClientRef.current.subscribe(`/topic/orders/${orderId}/status`, (message: { body: string }) => {
          try {
  ...
  useEffect(() => {
    ...
    return cleanup;
  }, [currentOrderId, startShortPolling, connectSSE, connectWebSocket, cleanup, resetOrderTrackingInfo]);
  ...
  return (
    <Card className="order-tracking shadow-sm mt-6">
      <CardHeader>
        <CardTitle className="text-center text-blue-600">Order Tracking</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <div className="space-y-4">
          <div className="border-b border-gray-200 pb-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h6 className="text-gray-600 font-medium">Short Polling Updates</h6>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-500">Status:</span>
              <span className={getOrderStatusBadgeClasses(orderStatuses.shortPolling)}>
            ...
            {canceledReasons.shortPolling && (
              <div className="mt-3">
                ...
                <p className="text-red-600 mt-1">{canceledReasons.shortPolling}</p>
              </div>
            )}
          </div>
          ...
          <div className="border-b border-gray-200 pb-4">
            ...
            <div className="flex justify-between items-center">
              <span className="text-gray-500">Status:</span>
              <span className={getOrderStatusBadgeClasses(orderStatuses.sse)}>
                {orderStatuses.sse}
              </span>
            </div>
            {canceledReasons.sse && (
          ...
          <div>
            ...
            <div className="flex justify-between items-center">
              <span className="text-gray-500">Status:</span>
              <span className={getOrderStatusBadgeClasses(orderStatuses.ws)}>
                {orderStatuses.ws}
              </span>
            </div>
            {canceledReasons.ws && (
