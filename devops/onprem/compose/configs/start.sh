...
# Compose file paths
BACKEND_COMPOSE_FILE="$PROJECT_ROOT/deployment/onprem/compose/docker-compose.backend.yml"
INFRA_COMPOSE_FILE="$PROJECT_ROOT/deployment/onprem/compose/docker-compose.infra.yml"
OBSERVABILITY_COMPOSE_FILE="$PROJECT_ROOT/deployment/onprem/compose/docker-compose.observability.yml"
...
# Logger
if [ ! -f "$PROJECT_ROOT/devops/scripts/lib/logger.sh" ]; then
  echo "Error: logger.sh not found at $PROJECT_ROOT/devops/scripts/lib/logger.sh"
  exit 1
fi
